...
def record(kernel, rootfs, timeout, record_func=__record, output="record", additional_args=None):
    if not has_snapshot(rootfs):
        print("No booted snapshot found. Booting once...")
        snapshot(rootfs, kernel)

    args = [rootfs, output, timeout]
    if additional_args:
...
def replay(rootfs, kernel, record, replay_func=__replay, additional_args=None):
    ...
    if p.exitcode:
        raise Exception(f"Replay failed {p.exitcode}")
